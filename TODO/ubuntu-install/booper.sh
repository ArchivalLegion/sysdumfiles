...
set +e
echo "Making sure zfs services are enabled" && {
...
echo "Trigger cache refresh" && {
	zfs set canmount=on bpool/BOOT/"$DSET"
	zfs set canmount=on rpool/ROOT/"$DSET"
	}
echo "Terminate zed" && {
	killall zed
	}
...
echo "Create inital system snapshot" && {
	zfs snapshot bpool/BOOT/"$DSET"@bpool_install
	zfs snapshot rpool/ROOT/"$DSET"@rpool_install
	}
